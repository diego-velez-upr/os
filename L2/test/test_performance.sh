...
# ------------- Test 3: Hamlet (word-based content; supports single or multiple printf) -------------
...
# Read output into an array to allow lookahead/accumulation
mapfile -t __lines < <(printf "%s\n" "$output")

# Helper regex (bash [[ =~ ]] style)
actor_prefix_re='^Actor \(PID: [0-9]+\): *'
director_prefix_re='^Director \(PID: [0-9]+\):'

for ((i=0; i<${#__lines[@]}; i++)); do
  line="${__lines[i]}"

  if [[ "$line" =~ $actor_prefix_re ]]; then
    # Start accumulating the entire Actor turn until the Director speaks
    buf=""
    # From this Actor line forward…
    for ((j=i; j<${#__lines[@]}; j++)); do
      seg="${__lines[j]}"

      # Stop when Director speaks (next turn)
      if [[ "$seg" =~ $director_prefix_re ]]; then
        break
      fi

      if [[ "$seg" =~ $actor_prefix_re ]]; then
        # Strip "Actor (PID: N):" and optional spaces
        seg="$(sed -E 's/^Actor \(PID: [0-9]+\): *//' <<<"$seg")"
      fi

      # Append with a space (join multi-line printf chunks)
      buf+="$seg"$'\n'
    done

    # Normalize the concatenated Actor turn
    buf_one_line="$(printf '%s' "$buf" | tr '\n' ' ')"
    text_norm="$(printf '%s' "$buf_one_line" | normalize_words)"
    best_sample="$text_norm"

    # Content check (substring): students may add extra words before/after
...
if $hamlet_ok; then
  ...
  echo -e "${RED}  - No Actor turn whose content (ignoring punctuation/quotes) contains the expected verse.${NC}"
